...
colors:
  ...
  navy-surface:     "#173060"
  navy-border:      "#1e3d72"
  navy-border-hi:   "#2a52a0"
  ink-bright:       "#eaf2ff"
  ink-mid:          "#a8c8f0"
  ink-dim:          "#7aa0d0"
  accent-blue:      "#4aacff"
  accent-blue-hi:   "#80c8ff"
  teal:             "#38bdf8"
  green:            "#34d399"
  amber:            "#fbbf24"
  rose:             "#fb7185"
...
**Key Characteristics:**
- Monospace numerals for anything that is a rank, count, or stat — DM Mono is the "this is real data" signal.
...
### Primary
- **Electric Blue** (`#4aacff` / `accent-blue`): the default theme's single accent — active states, links, primary buttons, current-rank highlight.
- **Blue Glow** (`#80c8ff` / `accent-blue-hi`): hover/active brightening of the primary accent, never a separate meaning.
...
### Neutral
- **Void** (`#08121e` / `navy-void`): page background.
- **Deep / Panel** (`#0e1e34`, `#122444`): layered surfaces — nav bars, modal backgrounds.
- **Surface** (`#173060`): raised cards, controls, the chart-size bar.
- **Border / Border-hi** (`#1e3d72`, `#2a52a0`): default hairline vs. focus/hover border.
- **Ink Bright / Mid / Dim** (`#eaf2ff`, `#a8c8f0`, `#7aa0d0`): primary text, secondary labels, tertiary/disabled text — in that order of emphasis.
...
**Data/Label Font:** DM Mono (with SFMono-Regular, monospace fallback)
**Accent Font:** Instrument Serif italic — a single tagline moment on the landing screen ("Your listening history. Your charts. Your life's soundtrack."), never used elsewhere.

**Character:** A serif display face gives section headers a touch of editorial warmth, while the DM Sans/DM Mono pairing (same foundry family, deliberately related) keeps UI text and data numerals visually consistent — sans for prose and labels, mono for anything that's a number or a rank.
...
- **Data** (500 weight, 0.875rem, 1.3 line-height): rank numbers, play counts, dates, size/period controls — always DM Mono.
- **Label** (500 weight, 0.75–0.8125rem, tight tracking): buttons, nav pills, chips — DM Mono, occasionally DM Sans for longer button text.

### Named Rules
**The Mono-Means-Data Rule.** If a piece of text is a number, a rank, a date, or a control label tied to data (size buttons, period nav), it renders in DM Mono. DM Sans is reserved for prose and long-form copy. Never mix the two within the same numeric readout.
...
- **Primary:** accent-colored background or border, DM Mono label, compact padding (~8px 14px).
...
### Chips / Pills
- **Style:** 20px radius, DM Mono label, used for period-nav segments and badge pills.
...
### Navigation
- **Period-nav / date-nav:** horizontal button rows, DM Mono labels, shrink variant (`.nav-shrunk`) for narrow viewports rather than wrapping.

## 6. Do's and Don'ts

### Do:
- **Do** use DM Mono for every number, rank, date, and data-tied control label.
...
- **Don't** mix DM Sans and DM Mono within the same numeric readout.
